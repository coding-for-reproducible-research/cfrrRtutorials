...
We can visualise this estimated model, by generating a scatter plot of the observed data and adding the fitted regression line to the plot. This can be done simply by passing the output of the `lm` function into the `abline` function. 
...
In the histogram above we can see a non-symmetrical distribution with a hard boundary of 0 on the left and a long tail on the right hand side. 
...
* how categorical variables are modelled in regression,
* logistic regression,
