...
Instead for for variables classified as having random effects, we are assuming that each group within that variable has it's own effect and that across all the groups the distribution of their effects is normal. For random effects we are interested in estimating the variance of the distribution from which the group effects come. Conceptually random effects must be categorical variables. 
...
$$H_{null}: \sigma_{u}^2 = 0$$
$$H_{alternative}: \sigma_{u}^2 \neq 0$$
...
As with all statistical tests, the ability to calculate estiamtes of teh parameters and perform significance testing relies of assumptions about the data you are using. For mixed effects regression these are:
...
### Graphical representation of mixed effects regression model
