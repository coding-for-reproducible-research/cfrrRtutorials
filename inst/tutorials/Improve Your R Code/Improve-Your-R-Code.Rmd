...
A style guide is a set of rules that embed consistency into your code. One such set of rules are contained in the [Tidyverse Style Guide](https://style.tidyverse.org/)^[Note, our course 'Working With Data in R' covers the main functionality of the Tidyverse.]. These rules are essentially just *opinions*, since there are no hard-and-fast rules to the way you write your code, and there are many different style guides available. We have chosen to discuss the Tidyverse style guide because of its popularity. We could not feasibly cover every point in the Tidyverse Style Guide within this session, but we hope to emphasise some of the main points and tie them together with an example so you can bring the ideas back to your own code after the session. Complete details can be found in the [style guide](https://style.tidyverse.org/). 

Being conscious of consistency in your code can go a long way in improving the ease of collaboration and thus the impact of your code on your research. We will approach consistency from three levels of increasing granularity:
...
The first very simple, yet very important, aspect of an R file is its name. When naming your file you should:
...
Often, using comments such as `# ---------- ... ------------` can be useful to increase readability and indicate what the purpose of each sub-section in the script is doing. For example, one way of organising a small script could be as follows:
...
Note, in R using the 'project' structure (add link) or even creating your own R package (add link) can be useful for more involved analyses, though we do not cover those topics here.
...
#### Functions
Functions are an integral part of R.
